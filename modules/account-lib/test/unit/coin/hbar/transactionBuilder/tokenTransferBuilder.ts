...
describe('HBAR Token Transfer Builder', () => {
  ...
  const initTxBuilder = () => {
    ...
    txBuilder.amount('10');
    txBuilder.tokenName(TOKEN_NAME);
    return txBuilder;
  };
  ...
  describe('should fail', () => {
    ...
    it('a token transfer transaction with an invalid token name', () => {
      const txBuilder = factory.getTokenTransferBuilder();
      assert.throws(
        () => txBuilder.tokenName('invalidtokenName'),
        (e) => e.message === 'Invalid token name',
      );
    });

    it('a token transfer transaction without destination param', async () => {
      const txBuilder = factory.getTokenTransferBuilder();
    ...
    it('a token transfer transaction without amount', async () => {
      const txBuilder = factory.getTokenTransferBuilder();
      txBuilder.fee({ fee: '1000000000' });
      txBuilder.source({ address: testData.ACCOUNT_1.accountId });
      txBuilder.to(testData.ACCOUNT_2.accountId);
    ...
    it('a token transfer transaction without token id', async () => {
      const txBuilder = factory.getTokenTransferBuilder();
      txBuilder.fee({ fee: '1000000000' });
      txBuilder.source({ address: testData.ACCOUNT_1.accountId });
      txBuilder.to(testData.ACCOUNT_2.accountId);
      txBuilder.amount('10');
      await txBuilder.build().should.be.rejectedWith('Invalid transaction: missing token id');
    });
