...
export default class Ecdsa {
  ...
  signConvert(shares: SignConvert): SignConvertRT {
    ...
    if (shareToBeSend['k']) {
      ...
      const beta0 = BigInt(100); // @TODO BG-50178 Bring random int here
      ...
      const nu0 = BigInt(100); // @TODO BG-50178 Bring random int here
  ...
  signCombine(shares: SignCombine): SignCombineRT {
    ...
    const delta = Ecdsa.curve.scalarAdd(Ecdsa.curve.scalarMult(k, gamma), Ecdsa.curve.scalarAdd(alpha, beta)); // Needs recheck here
  ...
  sign(M: Buffer, oShare: OShare, dShare: DShare): SignRT {
    const m = createHash('sha256').update(M).digest(); // recheck endiness here
