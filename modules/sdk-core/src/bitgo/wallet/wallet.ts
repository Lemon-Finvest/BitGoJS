...
export class Wallet implements IWallet {
  ...
  private async sendManyTss(params: SendManyOptions = {}): Promise<any> {
    ...
    // TODO: Remove conditional when moved to txRequestFull for everything
    if (this._wallet.type === 'custodial') {
      this.bitgo
        .post(
          this.bitgo.url('wallet/' + this._wallet.id + '/txrequests/' + signedTransaction.txRequestId + '/transfers', 2)
        )
        .send();
    }
